...
import type { Appointment, DoctorProfile, Patient } from "@/lib/types";
...
export default async function AppointmentsPage() {
  const [appointments, doctors, patients] = await Promise.all([
    ...
    apiFetch<DoctorProfile[]>("/doctors"),
  ...
  return (
    <div className="space-y-6">
      ...
      <BookAppointmentForm doctors={doctors} patients={patients} />
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
            ...
            {appointments.map((appt) => (
              <tr key={appt.id}>
                <td className="px-5 py-3 font-medium text-slate-900">{appt.patient.name}</td>
